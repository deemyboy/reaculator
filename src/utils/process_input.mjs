...
const repairStack = {
  ...
  NUM1_INVISIBLE_ZERO_WITH_OPERATOR_CATCHER: function (input) {
    // console.log(`NUM1_INVISIBLE_ZERO_WITH_OPERATOR_CATCHER ${input}`);
  ...
  NUM1_TRAILING_DECIMAL_ZERO_CATCHER: function (input) {
    ...
    let _value = input;
    let matches = _value.match(patternStack.NUM1_TRAILING_DECIMAL_ZERO_CATCHER);
    if (matches) {
      if (matches[2]) {
  ...
  NUM2_REPEATED_INITIAL_ZERO_CATCHER: function (input) {
    ...
    let _value = input;
    let matches = _value.match(patternStack.NUM2_REPEATED_INITIAL_ZERO_CATCHER);
    if (matches) {
      _value = _value.replace(/.$/, "");
      input = _value;
    }
  ...
  NUM2_REPEATED_ZERO_IN_DECIMAL_WITH_OPERATOR_CATCHER: function (input) {
    // console.log(`NUM2_REPEATED_ZERO_IN_DECIMAL_WITH_OPERATOR_CATCHER ${input}`);
  ...
  DOUBLE_OPERATOR_CATCHER: function (input) {
    ...
    return (
      input.substring(0, input.length - 2) + input.charAt(input.length - 1)
    );
  },
